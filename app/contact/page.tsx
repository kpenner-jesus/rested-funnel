...
export default function ContactPage() {
  ...
  const roomSubtotal    = SITE_CONFIG.rooms.reduce((s, r) => s + r.pricePerNight * (roomCounts[r.sku] ?? 0) * nights, 0);
  const meetingSubtotal = SITE_CONFIG.meetingRooms.reduce((s, r) => s + r.pricePerDay * (meetingRoomCounts[r.sku] ?? 0) * nights, 0);
  const mealSubtotal = (() => {
    let t = 0;
  ...
  const activitySubtotal = SITE_CONFIG.activities.reduce((s, a) => s + a.price * (activityCounts[a.sku] ?? 0), 0);
  ...
  const roomsSummary     = SITE_CONFIG.rooms.filter((r) => (roomCounts[r.sku] ?? 0) > 0).map((r) => `${r.name} x${roomCounts[r.sku]} (${nights} nights) = $${fmt(r.pricePerNight * roomCounts[r.sku] * nights)}`).join("\n") || "None selected";
  const meetingSummary   = SITE_CONFIG.meetingRooms.filter((r) => (meetingRoomCounts[r.sku] ?? 0) > 0).map((r) => `${r.name} x${meetingRoomCounts[r.sku]} (${nights} days) = $${fmt(r.pricePerDay * meetingRoomCounts[r.sku] * nights)}`).join("\n") || "None selected";
  const activitiesSummary = SITE_CONFIG.activities.filter((a) => (activityCounts[a.sku] ?? 0) > 0).map((a) => `${a.name} - ${activityCounts[a.sku]} participants = $${fmt(a.price * activityCounts[a.sku])}`).join("\n") || "None selected";
  const mealsSummary = (() => {
    const mc: Record<string, number> = {};
    for (const [k, s] of Object.entries(selectedMeals)) {
      if (s) { const mk = k.split("_")[1]; mc[mk] = (mc[mk] ?? 0) + 1; }
    }
    const labels: Record<string, string> = { breakfast: "Breakfast", lunch: "Lunch", supper: "Supper", nightsnack: "Night Snack", nutritionbreak: "Nutrition Break" };
    return Object.entries(mc).map(([k, d]) => `${labels[k] ?? k} x ${d} days`).join("\n") || "None selected";
  })();
  const taxBreakdown = SITE_CONFIG.taxes.map((t) => `${t.name} (${(t.rate * 100).toFixed(0)}%): $${fmt(pretax * t.rate)}`).join(", ");
  ...
  const handleSubmit = async () => {
    ...
    const SERVICE  = "service_z5vr2hr345";
    const TEMPLATE = "template_x5qjdb7t6";
    const GUEST    = "template_0sgecbx";
    const PUBKEY   = "Sr8ne3azvu5WL5xCk";

    emailjs.init({ publicKey: PUBKEY });

    emailjs.send(SERVICE, TEMPLATE, params).then(() => {
      emailjs.send(SERVICE, GUEST, { ...params, to_email: email.trim() }).then(() => {
        setSent(true);
        setTimeout(() => router.push("/quote"), 1800);
      }).catch((err2: any) => {
        console.error("Guest email error:", err2);
        setSent(true);
        setTimeout(() => router.push("/quote"), 1800);
      });
    }).catch((err: any) => {
  ...
  if (sent) return (
    <div className="tf-step">
      <div className="tf-body" style={{ justifyContent: "center", alignItems: "center", textAlign: "center" }}>
        ...
        <h2 style={{ fontFamily: "var(--font-display)", fontSize: "2rem", fontWeight: 500, marginBottom: "0.75rem" }}>Enquiry received!</h2>
        <p style={{ color: "var(--text-secondary)", maxWidth: "340px", lineHeight: 1.6 }}>
          Thanks, {name.split(" ")[0]}. We sent your quote to <strong>{email}</strong> and notified our team. A coordinator will follow up within 1–2 business days.
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="tf-step">
      ...
      <div className="tf-body">
        ...
        <h1 className="tf-question tf-animate tf-animate-delay-1">Almost done — <em>who are you?</em></h1>
        <p className="tf-subtext tf-animate tf-animate-delay-2">We will send your quote here and follow up within 1–2 business days.</p>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
          {[
            ...
            <div key={id} className={`tf-animate tf-animate-delay-${i + 3}`}>
              ...
              <input type={type} value={val} onChange={(e) => { (set as any)(e.target.value); setError(""); }} placeholder={ph} className="tf-input-box" />
            </div>
          ))}
          <div className="tf-animate tf-animate-delay-6">
            ...
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Dietary needs, accessibility requirements, flexible dates…" rows={3} className="tf-input-box" />
          </div>
        </div>
        ...
        <p style={{ fontSize: "0.72rem", color: "var(--text-muted)", marginTop: "0.875rem", fontWeight: 300 }}>Your information is only used to follow up on this enquiry.</p>
